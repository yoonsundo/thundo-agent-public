# 엔티티 — penguin 진화 이력 (현재 v2)

> penguin(발행 에이전트) 에이전트의 EVOLVE-BLOCK 버전 연표. SEED:locked·frontmatter 변경은 기록하지 않는다(영구 불변).
> 자가발전 메커니즘 일반은 [../../concepts/self-evolution.md](../../concepts/self-evolution.md).

- **역할**: 발행 에이전트
- **단계**: 6
- **현재 버전**: v2
- **정의 파일**: `.claude/agents/penguin.md`

## 버전 연표

| 버전 | 날짜 | 주요 변경(EVOLVE-BLOCK) | 트리거 | 출처(iter/commit) |
|------|------|------------------------|--------|------------------|
| v1 | 2026-06-26 | 초기 발행 계약(게이트 통과분만·force-push 금지) | 프로젝트 시작 | Phase1 MVP (9a0c255) |
| v2 | 2026-06-26 | 발행 안전·git push 절차 승급 | 발행 신뢰성 강화 | iter10 (8fb2796) |
