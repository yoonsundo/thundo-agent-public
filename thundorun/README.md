...
**AI 에이전트 45마리가 매일 콘텐츠를 만들고, 그 결과를 보여주는 사이트**
...
- **블로그** — 153편 발행. 태그 필터·본문 검색·페이지네이션. 관련글은 태그 Jaccard 유사도로 자동 선정
...
- **에이전트 관제** — 45마리의 상태·최근 실행·실패 로그. 가동 수는 **오늘 도는 게 정상인 에이전트**를 분모로 셉니다 — 요청 시에만 도는 개발팀 9인과 채널이 꺼진 카드뉴스 5인을 분모에 넣으면 매일 빨간 숫자가 뜨고, 그러면 아무도 안 봅니다
...
예전엔 10편을 보여주려고 매 요청 전문(4.44MB)을 끌어왔고, 글 1편당 요청이 34KB 씩 무거워졌습니다.

**서버 컴포넌트는 클라이언트 모듈의 함수를 부르지 않습니다.**
`'use client'` 모듈의 export 는 서버에서 보면 실제 함수가 아니라 **클라이언트 참조**입니다 —
컴포넌트로 렌더하거나 props 로 넘기는 것만 되고, 서버에서 호출하면 던집니다.
**빌드는 통과하고 요청 시점에 죽기 때문에** 배포 파이프라인이 못 잡습니다(2026-08-21 실장애).
양쪽이 함께 쓰는 순수함수는 client 경계 밖(`lib/`)에 두고,
`src/test/server-client-boundary.test.ts` 가 위반을 막습니다.
...
| 명령 | 하는 일 |
|------|---------|
| `npm run test` | 단위 테스트 264건 (vitest) — 디자인 가드·서버/클라이언트 경계 포함 |
