...
| 8 | **이모지 금지.** 아이콘은 [Lucide](https://lucide.dev) (`lucide-react`)만 사용, 크기 16/18/20px — 단 에이전트 마스코트 마크는 예외(§12.12). **이모지 금지는 예외 없음** |
...
| 11.15 | `.run-scope` | `/run` 이미지 편집기 격리 래퍼(`src/app/run/layout.tsx`). 사이트 UI 개편에서 이 화면만 현행 유지한다 — 편집기는 캔버스·포인터 조작이 얽혀 전역 스타일이 바뀌면 조작감이 깨진다. `display: contents` 라 **레이아웃에 참여하지 않고**(편집기 최상위가 `height:100dvh` + flex 열이라 래퍼가 박스를 만들면 높이 사슬이 끊긴다) CSS 변수 상속과 하위 선택자만 살린다. 안에서 `--dur-*: 0ms` · `--press-scale: 1` 로 모션 토큰을 되돌린다. ⚠ 이 격리가 성립하려면 **새 모션을 반드시 토큰으로** 써야 한다 — 값을 직접 박은 `transition` 은 스코프가 못 막는다. ⚠ 편집기 자체 모션은 건드리지 않는다(일괄 `transition:none` 금지). 래퍼를 `page.tsx` 가 아니라 별도 레이아웃으로 둔 이유는 **동결 8파일 무수정**을 지키기 위해서다 |
| 11.16 | `.video-thumb` `.agent-grid` `.agent-chip` `.reveal` | 홈 좌측 열 보강(2026-08-19). 인기글 5개만 있어 1440px 에서 화면 절반이 비고 우측 사이드바와 높이가 크게 어긋났다 — 최근 영상(9:16 썸네일)과 에이전트 팀 미리보기로 채운다. `.agent-chip` 은 이름·역할을 세로로 쌓되 긴 값이 칩을 밀지 않게 안쪽 블록의 최소크기를 푼다. `.agent-grid` 의 최소폭 168px 는 실측값이다 — 키트 `.grid-auto-sm`(104px)로는 칩이 113px 이 되어 8개 **전부** 이름이 잘렸다(필요폭 = 가장 긴 이름 93px + 아바타 30 + gap 8 + padding 16 = 147px). `.reveal` 은 스크롤 진입(뷰포트 진입 시 1회, `transform`+`opacity` 만, 300ms 미만) — ⚠ JS 가 죽으면 콘텐츠가 투명한 채 남으므로 `.no-js .reveal` 로 기본 노출을 보장하고, 관찰자만으로는 화면을 건너뛴 요소를 못 깨우니 스크롤 훑기와 타임아웃 안전망을 함께 둔다 |
| 11.17 | `.card-quiet` | 부수 정보 카드(에이전트 팀·기술 스택). 2026-08-19 표면 개편으로 `.card` 가 흰색으로 떠오르면서 **모든 카드가 같은 무게**가 됐다 — 주력(인기글·영상)과 부수를 구분하려고 배경에 눕히는 변형을 둔다. ⚠ 전부 흰색이면 위계가 다시 사라진다는 것이 이 클래스의 존재 이유다 |
...
3. **아이콘은 `lucide-react`.** 이모지·기하문자(`◈` `✎` `🦁` …)를 UI에 쓰지 않는다. 크기 16/18/20px.
   - 예외 1건: **에이전트 마스코트 마크**(§12.12). 아이콘이 아니라 브랜드 자산이며 `.avatar` 안에서만 쓴다. 이모지 금지에는 예외가 없다.
...
12. **에이전트 마스코트 마크는 규칙 8(아이콘은 lucide 전용)의 예외다.**

    2026-08-21 사용자 결정. 에이전트 43명의 정체성은 lucide 어휘로 표현할 수 없다 —
    설치본(`lucide-react`)의 동물 아이콘은 19개뿐이라 우리 새 계열 15마리(팰컨·이글·크레인·스완·
    레이븐·피콕·펭귄·맥파이·로빈·나이팅게일·허밍버드·구스·헤론·패럿·우드페커)가 전부 `Bird`
    하나로 붕괴한다. 이름 첫 글자로 대신하던 기존 방식도 **6개 군 13명이 충돌**한다
    (라=라이언·라이노·라쿤, 비=비버·비, 스=스완·스파이더, 배=배저·배포담당, 디=디자이너·디어,
    헤=헤론·헤지호그).

    따라서 마스코트 마크는 **아이콘이 아니라 브랜드 자산**으로 취급하고 인라인 SVG 로 둔다.
    조건 4가지를 지킨다:
    - ① **lucide 시각 문법 준수** — 24 viewBox · stroke 1.5~2 · 라운드 캡/조인.
      한 화면에 두 시각 언어를 두지 않기 위한 조건이다.
    - ② **색은 `currentColor` 만.** 색 리터럴 금지 — 라이트/다크가 이것 하나로 동시에 풀린다
      (§12.4 가 컴포넌트 다크 분기를 금지하므로 선택이 아니라 요구사항이다).
    - ③ **`.avatar` 컨테이너 안에서만** 사용한다. 본문 아이콘 자리에 쓰지 않는다.
    - ④ 크기는 `size={}` 가 아니라 **`width`/`height`** 로 준다. 규칙 8 보강의 16/18/20 스케일은
      **본문 아이콘** 규칙이며 아바타 안 마크는 그 대상이 아니다.

    ⚠ 이 예외는 **이모지 금지를 완화하지 않는다.** DB 에 `emoji` 필드가 있어도(`agents.sql`)
    UI 는 영구히 렌더하지 않는다 — 규칙 8 위반인 데다 시드 데이터에 중복이 4쌍이라
    (치타·팬서 `🐆` / 팰컨·이글 `🦅` / 폭스·페넥 `🦊` / 맥파이·로빈 `🐦`) 애초에 식별 문제를
    풀지도 못한다. **"필드가 있는데 왜 안 쓰지?" 하고 되살리지 말 것.**

    개발팀 9명(지휘자·기획자·설계자·디자이너·코더·테스터·보안리뷰어·검증자·배포담당)은 동물이
    아니므로 이 예외의 대상이 아니다 — **lucide 역할 아이콘을 그대로 쓴다.**
