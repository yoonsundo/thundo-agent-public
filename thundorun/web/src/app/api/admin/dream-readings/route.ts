...
export async function GET(req: NextRequest) {
  ...
  const { data, count, error } = await db
    .from('dream_readings')
    .select('id, dream_text, mood, verdict, lucky_numbers, symbol_tags, result, created_at', { count: 'exact' })
    .order('created_at', { ascending: false })
    .range(from, from + limit - 1);

  // 테이블이 아직 없을 경우 어드민 화면이 죽지 않도록 graceful fallback
  if (error) {
    return NextResponse.json({ items: [], total: 0, configured: true, tableMissing: true });
  }
...
export async function DELETE(req: NextRequest) {
  ...
  const { data, error } = await db.from('dream_readings').delete().eq('id', id).select('id');
